...
label genuinely new proposals as new.
...
Show only the states, viewport sizes, keyboard behavior, or failure paths relevant to the decision.
Keep the artifact inspectable and capture the developer's decision and rationale.

A later implementation request grants product mutation authority; the mockup does not need its own
approval state.
