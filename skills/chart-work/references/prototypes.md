...
routine implementation, or a preference answerable directly needs no prototype. This is a decision
rule, not a required phase for every task.
...
artifact, observation that would settle it, and a bounded effort budget before starting; no separate
approval is needed inside existing authority. Ask before crossing that authority, not merely
because the artifact is called a prototype.
...
growing the experiment. Keep experimental code isolated from production paths. Retain useful
evidence; promoting code to production requires the normal implementation and review contract.
