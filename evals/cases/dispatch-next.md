...
| DN-P4 | The queue contains three implementation-ready issues and the developer asks to dispatch the next task without requesting a batch. | It selects one issue and routes that issue to `deliver-work`; it does not infer batch-work from the queue size. |
